...
class Computer:
    ...
    def get_dict(self):
        """ Представление объекта в формате словаря """
        data = {
            "name": self.name,
    ...
    def load(cls, filename):
        ...
        return Computer.from_dict(json_data)
...
class Smartphone(Computer):
    ...
    def load(cls, filename):
        ...
        with open(filename) as json_file:
            json_data = json.load(json_file)
        return Smartphone.from_dict(json_data)
...
class PersonalComputer(Computer):
    ...
    def load(cls, filename):
        ...
        with open(filename) as json_file:
            json_data = json.load(json_file)
        return PersonalComputer.from_dict(json_data)
...
class DesktopComputer(PersonalComputer):
    ...
    def from_dict(cls, json_data):
        ...
        if "pc_os" in json_data:
            pc_os_json = json_data["pc_os"]
            if "os_type" in pc_os_json and "os_version" in pc_os_json:
                os_members = DesktopComputer.OperatingSystem.Type.__members__
                os_type_dict = {name: member
                                for name, member in os_members.items()}
                if pc_os_json["os_type"] in os_type_dict:
                    os_type = os_type_dict[pc_os_json["os_type"]]
                    pc_os = DesktopComputer.OperatingSystem(
                        os_type, pc_os_json["os_version"])
                else:
                    enough_data = False
            else:
        ...
        if not enough_data:
            return None
        pc = DesktopComputer(name, manufacturer, model, pc_os)
        if "builtin_memory" in json_data:
            pc._builtin_memory = json_data["builtin_memory"]
        if "ram_memory" in json_data:
            pc._ram_memory = json_data["ram_memory"]
    ...
    def load(cls, filename):
        ...
        with open(filename) as json_file:
            json_data = json.load(json_file)
        return DesktopComputer.from_dict(json_data)
...
class Laptop(PersonalComputer):
    ...
    def from_dict(cls, json_data):
        ...
        if "pc_os" in json_data:
            pc_os_json = json_data["pc_os"]
            if "os_type" in pc_os_json and "os_version" in pc_os_json:
                os_members = Laptop.OperatingSystem.Type.__members__
                os_type_dict = {name: member
                                for name, member in os_members.items()}
                if pc_os_json["os_type"] in os_type_dict:
                    os_type = os_type_dict[pc_os_json["os_type"]]
                    pc_os = Laptop.OperatingSystem(
                        os_type, pc_os_json["os_version"])
                else:
                    enough_data = False
            else:
        ...
        if not enough_data:
            return None
        pc = Laptop(name, manufacturer, model, pc_os)
        if "builtin_memory" in json_data:
            pc._builtin_memory = json_data["builtin_memory"]
        if "ram_memory" in json_data:
            pc._ram_memory = json_data["ram_memory"]
    ...
    def load(cls, filename):
        ...
        with open(filename) as json_file:
            json_data = json.load(json_file)
        return Laptop.from_dict(json_data)
